...
#ifdef CONFIG_SCHED_EMS
		overutilized = ems_lbt_overutilized(rq->cpu, sd->level);
#else
...
#ifdef CONFIG_SCHED_EMS
		if (ems_lbt_overutilized(i, env->sd->level)) {
#else
...
#ifdef CONFIG_SCHED_EMS
	return ems_need_active_balance(env->idle, sd, env->src_cpu, env->dst_cpu);
#else
	/*
